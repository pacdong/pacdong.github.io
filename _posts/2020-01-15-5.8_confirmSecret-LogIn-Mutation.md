...
`yarn add cors` 로 cors를 설치한 뒤에 BackEnd 서버에 아래와 같은 미들웨어를 추가하면 됩니다. 이 때 노마드 코더의 serve.js에 추가하도록 합니다.
...
```
다만 이렇게 모든 요청에 대해서 허가를 하게 되면 보안적으로 취약해질 수 있기에 cors Option을 통해 로컬 주소를 등록합니다.
...
```
이렇게 노마드 코더의 실습을 진행하면서 발생한 에러를 정리하였습니다.
